...
## 型についての補足
- 「型」とは変数に格納するデータ種別を予め決めるためのものです
- プログラミング言語によりますが、「型」には整数を表す`int`や、実数を表す`double`、文字を表す`char`などが存在し、宣言したデータ種別以外のデータを格納できません
- たとえば、Javaで次のような宣言を行うとエラーになります


    int foo = 0.1; // 整数で変数を宣言しているが、0.1は実数なのでエラー
    
- `Perl`ではこの「型」を指定する必要が無く，空気を読んでよしなに動作します。これは便利な一方で，予期せぬ結果になる場合もあるため注意が必要です


    print "1a" + 1; # => 2 数値として認識できた値を利用します
